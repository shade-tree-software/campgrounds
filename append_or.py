...
formatting; never full-rewrites). Usage: python3 append_or.py <results.json> [...]
...
/tmp/or_leads.json keyed by new id for the audit stage."""
import json, sys, os

CG = '/home/andrew/Dev/campgrounds/campgrounds.json'
LEADS = '/tmp/or_leads.json'
...
def main():
    paths=sys.argv[1:]
    if not paths:
        print('usage: append_or.py <results.json> [...]'); sys.exit(1)
    ...
    def norm(nm):
        ...
        return nm
    wa_coords=[]
    wa_names={}  # normalized-name -> list of (coord_or_None, id, name)
    for e in data:
        if e.get('state')=='OR':
            p=parse(e.get('location','') or '')
            if p: wa_coords.append((p,e['id'],e.get('name','')))
            n=norm(e.get('name',''))
            if n: wa_names.setdefault(n,[]).append((p,e['id'],e.get('name','')))
    ...
    for r in rows:
        ...
        if nn and nn in wa_names:
            ...
            # near (~1.5 km) -- distinct campgrounds share names across OR
            # (Fish Lake, Rock Creek, Eightmile, Big Pine, ...)
            namedup=None
            for (q,eid,enm) in wa_names[nn]:
        ...
        e={'id':next_id,'kind':'campground','name':r['name'],
           ...
           'state':'OR','ownership':r['ownership'],
        ...
        if nn: wa_names.setdefault(nn,[]).append((p,next_id,r['name']))
        if r.get('lead'):
