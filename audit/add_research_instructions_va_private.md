# Campground add-stage research — agent instructions (VA private re-sweep)
...
**Why this batch exists:** Virginia's private stage (2026-06-01) was detected from
**Good Sam**, with RV Life used only to filter that list on price/stars. Any
independent park without a Good Sam rating was therefore never a candidate — the
gap surfaced when Gooney Creek Campground (Front Royal, RV Life 5★/$$) turned out
to be missing. This batch is the RV-Life-first detection VA never got: all VA
...
- fairground / horse-show / event-lot that camps only during events
- **membership / club / sales-pitch park** — Thousand Trails, Encore, Coast-to-Coast,
  ...
  Several candidates in this sweep are exactly these; skip them and say which chain
  or club it is.
- **residential / mobile-home / seasonal-full-timer / workforce park; casino or
  truck-stop overnight lot**
...
**Two traps carry most of the risk in this bucket.**
1. **The club lodge.** An Elks/Moose lodge RV lot rates well on RV Life and is cheap,
   but it is members-and-reciprocal-guests only. That is a membership park — SKIP.
2. **The seasonal park.** A park that sells mostly seasonal sites is still a KEEP **if
   it genuinely holds a real inventory of nightly transient RV sites** — but if
   transient camping is a token handful, or the site map is wall-to-wall permanent
   trailers with decks/sheds/skirting and a "seasonal only" policy, SKIP it. Check the
   operator's own rates page for a nightly rate, and look at satellite.

When unsure between tent-only and RV-capable, dig into the operator's own site map or
booking engine. **Aggregators (snoflo, camperalerts, campscanner, thedyrt /
...
**A dead official domain is a strike, not an automatic skip.** Check where it actually
resolves — Gooney Creek's gooneycreek.com now 301s to an unrelated business while the
campground itself is live and instant-bookable on Hipcamp. If the domain is dead,
confirm operation from a live booking channel (Hipcamp, Campspot, RoverPass,
ReserveAmerica) or dated reviews, point `website` at that live channel, and record the
dead domain as a caveat in the `note`.
...
- **`state`** — Virginia State Parks (DCR), state forests, Breaks Interstate Park.
- **`local`** — county / town / city / regional-authority parks, including ones run
  for a county by a private concessionaire.
- **`federal`** — USACE / USFS / NPS / BLM.
- **`private`** — everything else: commercial parks, farm/winery campgrounds, church
  or club campgrounds open to the public, utility-run parks.
...
### Virginia reservation systems (for the `website` deep link)
- **VA State Parks** — ReserveAmerica, contractCode `VA`:
  `https://virginiastateparks.reserveamerica.com/campgroundDetails.do?contractCode=VA&parkId=<id>`
- **Federal** — `recreation.gov/camping/campgrounds/<id>`
- **Private** — the operator's own site, Campspot, Hipcamp, RoverPass, or phone.
...
- **`location`** — pin to the actual CAMPGROUND LOOP (where the pads are), 5 decimals,
  ...
  are often km off — Gooney Creek's put it on the highway at the entrance while the
  loops were 50 m east across the creek.
...
- **`website`** — official operator page first, then the deep reservation link
  ...
  stay as prose in the note: a NEGATIVE reservation fact ("FCFS — not on recreation.gov")
  and a dead/suspect official domain recorded as a caveat.
...
- **`note`** — concise, every claim sourced. Include the camping basics you confirmed
  ...
  aggregator boilerplate.
- **`inclusion_evidence`** — ONE line naming the authoritative source + what it showed,
  e.g. `Operator rates page + campground map: 42 nightly W/E sites to 45 ft alongside a
  seasonal section -> real transient RV campground`.
- **`waterfront`** — ALWAYS the literal placeholder `"not waterfront"` (a later audit
  decides; do NOT set it from name/marketing — several of these are named "Riverside",
  "Creek", "Cove" or "Bay" and that is explicitly NOT evidence).
- **`lead`** — head start for that later waterfront audit (NOT a verdict):
  `{"map_url": "<official per-site campground map/site-map url>",
    "water_body": "<named lake/river/bay or ''>",
    "candidate_sites": "<site numbers you saw at/near the water, or ''>",
    "note": "<one-line shoreline observation from the map/satellite, or ''>"}`.
  VA has Chesapeake Bay and Atlantic shore candidates, so the coastal-dunes /
  coastal-woods overrides can apply — note it in the lead if sites sit behind a real
  undeveloped shore dune field or shore forest.
