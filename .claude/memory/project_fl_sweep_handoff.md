...
**FL is COMPLETE** (state, federal, private, local — all added + waterfront-audited). As of 2026-06-18 every FL campground carries `waterfront_evidence` (or `--AWH`); 0 unaudited. Nothing outstanding for Florida.
- FL **legacy waterfront audit — DONE** (commits `cd29729`/`16dc0de`/`31dbba2`, ids 148–381, 23 entries, one-agent-at-a-time): all 23 confirmed `not waterfront`. No upgrades.
- FL **last item — DONE**: **Clay County Fairgrounds RV Park** added as id 4859 (commit `c5d563c`). Vetted as a real year-round transient park (48 full-hookup sites, 30-day max, closed only during the spring fair — not event-only); reclassified private → **local** (county-operated via Fairgrounds@claycountygov.com); waterfront-audited not waterfront. So the deferred "private candidate" landed as local.
- FL **state deferred items already DONE** (added in the state sweep): Karick Lake South = 4719; Tate's Hell State Forest = Cash Creek 4722 + County Line OHV 4723.
