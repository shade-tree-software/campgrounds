...
- **Home cards**: a `.event-card.bare.home-card` renders at the start and end of the timeline (only when `trip.timeline` is non-empty). They reuse the event-card markup with the red-house SVG inside `.event-icon.home-icon`. Meta line is `Reston, VA — {date}` using `trip.start` and `trip.end` respectively, plus a `<span class="home-time-display" id="home-{start|end}-time-display">` that holds the boundary time. If the trip has a manual `home_start_time` / `home_end_time` it renders server-side as `· HH:MM`; otherwise after the GPS track loads, JS fills the span with `· HH:MM (auto)` (and stashes the value on `window.HOME_{START,END}_TIME_AUTO` for the edit prompt). Admins get an Edit button that opens a prompt framed as an override — the prompt reads "GPS-derived time is X; set a manual override only if that looks wrong" so the manual fields are positioned as escape hatches, not the primary input. Manual values still take precedence in `buildAnchors()` and `trimHomeBoundaries()`.
