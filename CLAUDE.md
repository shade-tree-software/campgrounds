...
The site requires login globally. `_require_login_globally()` is registered as a `before_request` handler — any unauthenticated request is redirected to `/login?next=<path>`. Only `login` and `static` endpoints are exempt. On top of that, `_require_admin()` guards mutation endpoints (upload, edit, delete, reorder, move, campground CRUD) for write access. Templates use the `is_admin` Jinja variable and `IS_ADMIN` JS constant to conditionally render edit controls; drag-drop is fully disabled for non-admin authenticated users (no draggable attributes, no event listeners).

User accounts live in `users.json` (`{username: {password_hash, is_admin}}`, hashed with `werkzeug.security.generate_password_hash`). Admins manage users via **`/admin/users`** (`templates/users_manage.html`): create user, reset password, toggle admin role, delete user. The page uses a JSON API (`GET/POST /api/users`, `PUT/DELETE /api/users/<username>`). Self-protection rules in the API: cannot delete your own account, cannot remove admin from your own account. The legacy CLI command `python ekko_trips_app.py create-admin` is still available for bootstrapping the first admin (or recovering when no admin exists).
...
- **Force straight track (admin):** trips have an optional `force_straight_route` boolean (persisted in `trips.json`, set via the "Straight track only" checkbox in the trip-detail add-buttons row). When true, the GPS-track fetch is skipped entirely and only the legacy dashed straight-line route is drawn. Toggling reloads the page.
- **Auto-fallback to straight track:** even without the manual override, if the trip has campgrounds but no GPS ping ever lands within `TRACK_NEAR_STAY_KM` (5 km) of any stay's coordinates, the track is treated as irrelevant and rendering is skipped — the straight-line route remains as the only polyline. Trips with no stays (events-only) bypass this check.
- **GPS track overlay:** `/api/trips/<id>/track` fetches OwnTracks-style points from a separate timeline service (`TIMELINE_API_TOKEN` + primary `TIMELINE_TID`, with optional `TIMELINE_TID_ALT` filling per-day gaps where the primary tid logged nothing). Cached on disk under `trip_data/track_cache/<id>.json`; trips that ended >7 days ago serve from cache permanently. The frontend (`trip_detail.html`) trims home-radius puttering at the trip's first/last days *and* anything before the GPS-derived (or manually-overridden) home-start / after the home-end boundary — so morning-of-trip excursions that returned home before the real departure don't appear on the polyline. It then applies "blackout windows" for stays and timed events, and splices in stay/event timestamps as anchor points so the polyline passes through them. Blackout windows pair each stay/event's time range with its location and a `BLACKOUT_RADIUS_M` (1500 m) proximity check — this drops pings clustered at the campground/event but keeps travel-day pings on back-to-back stays. **Stay-outlier suppression** runs alongside the blackout filter: for each stay with known coords, the timestamps of raw pings within `BLACKOUT_RADIUS_M` of the stay are precomputed (across the whole track, not date-bounded — so a checkout-morning at-camp ping still anchors). A kept ping outside that radius is dropped if it has at-stay anchors on both sides (in time) AND the *active* span between those anchors is ≤ `STAY_OUTLIER_ACTIVE_MAX_S` (90 min), where active span subtracts any consecutive-raw-ping gaps ≥ `STAY_OUTLIER_SLEEP_GAP_S` (60 min) so an overnight sleep gap doesn't whitewash a morning spurious spike. This catches GPS spikes / cell-tower fixes that report sustained-but-wrong coordinates while the user was at the campground (e.g., 50-min stuck-location clusters, brief morning jumps). Without proximity the chained windows of consecutive stays would erase the entire trip's track. Anchor *timestamps* are derived from the raw track so they interleave with real data instead of clustering at midnight boundaries. Stays use `findPingTime()`: stay-start = first raw ping inside `BLACKOUT_RADIUS_M` of the stay during its window; stay-end = last such ping. HOME boundaries use `findHomeBoundaryTimes()`, which handles two OwnTracks usage patterns: (a) continuous logging — pings come in at the house overnight and again all evening on return, with occasional GPS jitter spikes past the 100 ft (`HOME_EXACT_RADIUS_M`) house radius; (b) on-demand logging — OwnTracks only fires up when the user leaves, so the first ping is post-departure, near home but not exactly at home. The rule for HOME-start: walk forward through trip-range pings tracking a candidate boundary; reset the candidate on every at-house ping (`< HOME_EXACT_RADIUS_M`); set the candidate on the first >100 ft ping (still within `HOME_TRIM_KM`, 1.5 km) after a reset. Each iteration where the candidate has been alive for `HOME_BOUNDARY_LOCK_S` (1 hour) commits it as `homeStartTst` — but the scan does NOT break, so a later sustained near-home re-entry can still wipe the candidate and a later real departure can produce a new committed value. **Sustained re-entry reset:** once the candidate has been set and the user has gone outside HOME_TRIM (`hasGoneAway = true`), accumulating `HOME_REENTRY_DWELL_S` (30 min) of continuous near-home pings without re-departing wipes the candidate (and clears `hasGoneAway`) — *but only* when the AWAY streak has been shorter than `HOME_REENTRY_MAX_AWAY_S` (12 h). Brief pass-throughs (e.g., dry cleaners) don't dwell long enough to trigger this. This catches the pattern where an early-day excursion's first AWAY would otherwise lock as the trip start: when the user comes back and stays near home, the morning candidate is wiped, and the afternoon real-departure becomes the locked value. **Trip-end return freeze:** the first near-home ping observed after a long AWAY streak (≥ `HOME_REENTRY_MAX_AWAY_S`) flips a `tripEndReturned` flag; once set, `homeStartTst` updates are blocked for the rest of the scan. This prevents end-of-trip home-hovering from overwriting the morning departure — important when HOME coords are slightly off (e.g., between deployments with diverging `home.json`) and at-house pings alternate between @HOME and NEAR classifications, which would otherwise let a late-trip NEAR-only stretch lock a new candidate. Pattern (a) locks at the post-morning-cluster departure (e.g., 8:49 am, confirmed an hour later); pattern (b) locks at the very first ping. Because the scan keeps running through trip-end, the trip-return at-house cluster harmlessly resets the live candidate to null but leaves `homeStartTst` (already committed earlier in the day) intact. HOME-end mirrors this by walking backward: when there's a trailing at-house cluster the boundary is its earliest member (= moment of arrival home); when no at-house cluster exists at the trip tail (pattern b at trip end), it falls back to the last in-range near-home ping. The walk-backward halts at the first non-at-house ping it encounters at the tail, so any earlier (mid-trip or pre-trip) at-house pings can't influence it. Trip-related stops within the home neighborhood (e.g., the dry cleaners) are intentionally treated as trip activity rather than "still at home." Falls back to 00:00 / day-after boundaries when no qualifying ping exists at all. User-specified `home_start_time` / `home_end_time` override the HOME derivation. Without this, two anchors at distant locations with the same midnight timestamp (HOME at trip-start and stay#1 at arrival, or back-to-back stay end/start anchors) would sort adjacent and draw a spurious straight line even when real GPS already covers that travel.
- **Per-point GPS markers:** when a GPS track is rendered, each kept ping also gets a tiny clickable circle marker (radius 3, navy stroke, white fill) bundled in a `gpsPointLayer`. The layer is hidden until the user zooms in to `GPS_POINT_MIN_ZOOM` (14) — `syncGpsPoints()` runs on `zoomend` plus `overlayadd`/`overlayremove` so toggling the GPS track in the layer control also toggles its dots. Clicking a circle opens a popup showing local time **at the recording location** and lat/lng to 5 decimals. Timezone resolution: server-side `_enrich_with_timezone()` (uses `timezonefinder`) attaches an IANA `tz` field to each cached ping when serving `/api/trips/<id>/track`; the frontend passes that to `Date#toLocaleString({timeZone: ...})` so a Pacific-coast ping shows in PDT/PST while an Eastern-coast ping shows in EDT/EST regardless of the viewer's browser timezone. The package is an optional dependency — if `timezonefinder` is not importable, enrichment is silently skipped and the popup falls back to browser-local time. `_enrich_with_timezone()` is idempotent and run on every cache read so existing cache files are backfilled in place on first hit.
...
- **Home cards**: a `.event-card.bare.home-card` renders at the start and end of the timeline (only when `trip.timeline` is non-empty). They reuse the event-card markup with the red-house SVG inside `.event-icon.home-icon`. Meta line is `Reston, VA — {date}` using `trip.start` and `trip.end` respectively, plus a `<span class="home-time-display" id="home-{start|end}-time-display">` that holds the boundary time. If the trip has a manual `home_start_time` / `home_end_time` it renders server-side as `· HH:MM`; otherwise after the GPS track loads, JS fills the span with `· HH:MM (auto)` for admins or `· HH:MM` for non-admins (and stashes the value on `window.HOME_{START,END}_TIME_AUTO` for the edit prompt). Admins get an Edit button that opens a prompt framed as an override — the prompt reads "GPS-derived time is X; set a manual override only if that looks wrong" so the manual fields are positioned as escape hatches, not the primary input. Manual values still take precedence in `buildAnchors()` and `trimHomeBoundaries()`.
...
- Admin **Edit** and **Upload Photos** buttons live in a `<span class="card-actions">` that's a flex sibling of `.stay-info`/`.event-info` inside the card header. On desktop they sit at the top-right corner (via `margin-left: auto; align-self: flex-start`); on mobile they drop to row 2 of a CSS-grid override (`grid-area: actions`, left-aligned under the info column). The **Remove All Photos** button (admin-only) lives at the bottom of `.photos-section` (a flex column) with `align-self: flex-end`, sitting right-aligned beneath the photo grid on both desktop and mobile. `uploadPhotosForItem(kind, idx)` triggers a hidden file input and posts to the existing upload endpoint, then reloads.
