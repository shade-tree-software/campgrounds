...
- **`waterfront` vocabulary:** on-water `lakefront`/`riverfront`/`creekside`/`pond`/`bayfront` (+ `coastal woods`/`coastal dunes`); view-only `lakeview`/`riverview`/`bayview`; else `none` (RV-sites-only rule, above).
