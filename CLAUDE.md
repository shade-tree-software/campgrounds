...
**Waterfront re-audit (in progress):** every pre-evidence-gate `waterfront` value (non-`not waterfront`, note not tagged `--AWH`) is being re-verified state by state, working eastward from IL. Methodology, subagent instructions, apply script, and the remaining-state queue live in `audit/` (see `audit/README.md`); per-entry evidence is recorded in the audit commit messages ("Audit all NN <ST> waterfront designations"), so checking a past designation is a `git log` grep.
