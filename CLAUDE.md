...
- **Auto-fallback to straight track:** the GPS layer is silently skipped (and the dashed straight-line route stays as the only polyline) when the GPS data doesn't actually cover the trip. Triggered when either (a) the `/track` fetch fails or returns nothing in the trip window, or (b) the gate fails. **Strict gate (completed trips):** passes only if at least one in-window ping is within `TRACK_NEAR_STAY_KM` (5 km) of a stay/event anchor. A phone that roamed around for non-trip reasons (trip 61: primary drove out on 6/16 for an unrelated errand but never went to the trip's stay) fails this — moved-away pings aren't enough on their own, because they can't be distinguished from non-trip activity after the fact. **Relaxed gate (in-progress trips, i.e. `now <= end-of-day(TRIP_END)`):** also passes if at least one in-window ping is more than `TRACK_NEAR_STAY_KM` (5 km) from HOME. Covers the in-progress case where the user is driving toward a distant sole anchor and watching the polyline build in real time before reaching it (first observed on a since-deleted test trip, formerly "trip 90"). The relaxation drops once TRIP_END's local day is past — by then the question "did the phone ever reach the destination?" has a definitive answer. There is no manual "force straight" toggle — these heuristics are the only mechanism. (Older `trips.json` entries may still carry a vestigial `force_straight_route: true`; it is read by nothing and survives only until the trip is next saved.)
...
- **Frontend track rendering:** the GPS overlay is the polyline of every returned ping inside the trip window (sorted by timestamp). The window's lower bound is `HOME_START_TIME` (manual) / auto-detected `homeStartTst` / start of `TRIP_START` local day, in that order; upper bound mirrors with `HOME_END_TIME` / `homeEndTst` / start of the day after `TRIP_END`. Pings outside that window are dropped from both the polyline and the per-point markers. No blackout/radius suppression. Markers for stays, events, home, and family render independently of the polyline. **Gap-fill:** before the polyline is built, the day-by-day morning→events→evening walk that backs the dashed "Straight route" overlay is rebuilt with a timestamp per anchor (`routeStops`). The polyline then injects those anchors into any period not covered by good pings: the leading gap (anchors before the first good ping) and trailing gap (after the last) are filled unconditionally — the only candidate there is usually HOME at the trip edge, coincident with the at-home boundary-pad ping so a well-tracked trip is unaffected; an interior gap between two consecutive good pings is filled only when it spans `GAP_FILL_MIN_S` (90 min) or more, so routine sparse OwnTracks logging doesn't add detours. This is the only anchor splicing — per-point markers still come from raw pings, not the injected anchors. **Boundary pad:** the polyline also pads by one ping just outside each cut so it opens with a "leaving home" leg and closes with an "arriving home" leg, but only when that ping is within `BOUNDARY_PAD_MAX_S` (90 min) of the cut. With a tight manual `HOME_START_TIME`/`HOME_END_TIME` window the nearest outside ping can be hours/days away (trip 87: a 15:51 at-home blip and a next-morning 08:34 ping) — padding it would draw a long stale jump, so beyond the cap the pad is skipped and the leading/trailing HOME-anchor gap-fill draws the edge leg instead. Continuous-logging trips (pad ping seconds away) are unaffected. When the GPS track loads, the dashed straight-line route is removed from the map (still toggleable via the layer control).
...
⚠ **Trip-boundary calculation lives in two places, and they share the same algorithm.** The home card's "(auto)" departure/arrival time uses `findHomeBoundaryTimes()` in `templates/trip_detail.html` (JS, ~line 2421). Stop detection uses `_find_home_boundary_tsts()` in `ekko_trips_app.py` (Python). Both use centroid-classified at-home runs with anchor-aware streak selection (near-anchor streak with the smallest closest-approach to an itinerary anchor wins; longest qualifying streak is the fallback when none reach an anchor or no anchors are passed). The JS side gets its anchors from `mapped` stays + `mappedEvents`; the Python side from `_anchors_for_trip(trip)`. Manual `trip.home_start_time` / `trip.home_end_time` win over auto-detected values on both sides. Aligned constants: frontend `HOME_TRIM_KM` (1.5) ↔ backend `STOP_NEAR_HOME_M` (1500); frontend `HOME_AT_HOME_CENTROID_M` (600) ↔ backend `STOP_AT_HOME_CENTROID_M` (600); frontend `HOME_BOUNDARY_LOCK_S` (3600) ↔ backend `STOP_HOME_BOUNDARY_LOCK_S` (3600); frontend `HOME_BOUNDARY_ANCHOR_M` (5000) ↔ backend `TRACK_NEAR_STAY_KM` (5 km). If you change behavior in one, change it in the other — the two implementations exist only because there's no shared lib between Python and the inline JS bundle, not because they should diverge.
