...
- **Manual ping suppression (admin):** trips carry an optional `suppressed_pings` list of `tst` integers. The track endpoint filters those points out before returning the polyline / regular markers; with `?include_suppressed=1` it instead tags each ping with `suppressed: true/false` so the admin "Show suppressed pings" toggle can flip visibility instantly without re-fetching. Suppressed pings render as gray dashed `circleMarker`s in a dedicated layer; clicking one DELETEs it from the suppressed list and reloads. Suppressing is a "Suppress" button in the selection toolbar that POSTs the selected `tst`s, then auto-unchecks Select pings (same flow as Event/Waypoint/Set-campsite-location). Cache stays raw — filtering happens on serve, so unsuppress is instant. Endpoints: `POST /api/trips/<id>/suppress-pings` body `{tst: [...]}` and `DELETE` same path.
...
- `GET /api/trips/<id>/track[?include_suppressed=1]` — GPS-track points; admin pages pass the flag so each ping is tagged `suppressed: true/false` instead of being filtered out
- `POST /api/trips/<id>/suppress-pings` — mark `tst`s as suppressed (body `{tst: [...]}`); idempotent
- `DELETE /api/trips/<id>/suppress-pings` — unsuppress (same body shape)
