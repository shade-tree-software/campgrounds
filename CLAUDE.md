...
- **Force straight track (admin):** trips have an optional `force_straight_route` boolean (set via "Straight track only" checkbox in the add-buttons row). When true, GPS fetch is skipped entirely. Auto-fallback: if no GPS ping lands within `TRACK_NEAR_STAY_KM` (5 km) of any stay, the GPS layer is skipped too and the straight route stays as the only polyline. Events-only trips bypass the auto-fallback check.
- **GPS track overlay:** `/api/trips/<id>/track` fetches OwnTracks-style points from a timeline service (`TIMELINE_API_TOKEN` + primary `TIMELINE_TID`, optional `TIMELINE_TID_ALT` filling per-day gaps). Cached on disk at `trip_data/track_cache/<id>.json`; trips ended >7 days ago serve from cache permanently. Spurious pings (cell-tower fixes, single jumps) are filtered upstream by a separate pre-processing app. Server-side `_enrich_with_timezone()` (optional `timezonefinder` dep, idempotent on cache read) attaches per-ping IANA `tz` so the per-point markers' popups can show local-at-recording-location time.
- **Frontend track rendering pipeline** (in `trip_detail.html`, all heavily commented inline — see those comments for invariants and edge cases):
  1. `findHomeBoundaryTimes()` derives HOME-start / HOME-end timestamps from the raw pings, handling two OwnTracks patterns (continuous logging vs. on-demand). Sustained-near-home re-entry can wipe a morning AWAY candidate so an early excursion doesn't lock as the trip start; a long-AWAY return freezes `homeStartTst` so trip-end home-hovering can't overwrite it. Manual `home_start_time` / `home_end_time` (admin overrides on the home cards) take precedence.
  2. `trimHomeBoundaries()` drops pings outside the resolved HOME-start/HOME-end window plus residual same-day home-radius puttering.
  3. Blackout filter: per-stay/per-timed-event windows pair time range with location and a per-window radius — `BLACKOUT_RADIUS_M` (1500 m) default, `STAY_OVERRIDE_BLACKOUT_RADIUS_M` (50 m) when a stay carries a `campsite_location` override. Pings inside a window AND inside its radius are dropped (the campground/event is "blacked out") unless a closer event coord rescues them.
  4. `buildAnchors()` splices structural anchors into the polyline so it visibly visits HOME and stays. Anchor timestamps come from real raw pings (via `findPingTime()` and the residency-cluster scan) so anchors interleave with kept pings instead of clustering at midnight and drawing spurious teleports.
  5. **No-cluster fallback diverges by override:** when the residency scan finds zero clusters for a stay, a non-override stay emits structural midnight anchors at the stay coord (so the polyline visibly visits camp). Override stays *skip* the fallback and `trackLog` a warning — emitting anchors at a likely-wrong override coord would teleport the polyline. Skipping makes the misalignment visible to admins so they can fix the override.
  6. Stay-outlier suppression: a kept ping outside the stay's blackout radius is dropped if it has at-stay anchors on both sides AND the *active* span between them is ≤ `STAY_OUTLIER_ACTIVE_MAX_S` (90 min, sleep gaps subtracted) AND no off-stay ping in the cluster reached beyond `STAY_OUTLIER_MAX_DIST_M` (2500 m). Catches stuck-location GPS clusters at the campground without erasing real short excursions.
- **Card-time tz interpretation:** wall-clock times entered on cards (event start/end, stay date midnight, home `home_start_time`/`home_end_time`) are interpreted in the location's local tz, NOT the viewer's browser tz. The trip-detail route calls `_stamp_trip_timezones()` after `enrich_trip_locations()` to attach an IANA `tz` to each stay/event and resolves `home_tz` from `home.json`; the client uses `localEpochInZone(date, time, tz)` for every wall-clock → epoch conversion. Falls back to browser-local when `tz` is missing.
- **Per-point GPS markers:** each kept ping also gets a small clickable circle (radius 3) inside `gpsPointLayer`, hidden until zoom ≥ `GPS_POINT_MIN_ZOOM` (14). Click shows local-at-ping time and lat/lng. Toggling the GPS track in the layer control also toggles the dots.
...
- Stay cards and regular event cards always render (with or without photos). Waypoints and family-visits without photos render as `.bare` cards: just the colored circle + header, transparent background, no body. Bare cards have a visible Edit button that opens the shared modal.
...
- **Home cards**: `.event-card.bare.home-card` renders at the start and end of the timeline. The `.home-time-display` span shows the manual `home_start_time`/`home_end_time` (server-rendered) when set, else the GPS-derived time appended client-side as `· HH:MM (auto)` for admins or `· HH:MM` otherwise. Admin Edit prompts are framed as overrides ("GPS-derived time is X; set a manual override only if that looks wrong").
