...
- **First-come-first-served:** always add a `note` saying so when there's no reservation system.
...
- **Waterfront reflects RV-accessible sites only.** Set an on-water value ONLY with site-level confirmation that some **RV** sites sit directly on the water — riverfront/lakefront cabins or tent-only/shelter sites do NOT qualify. Use a `*view` value only if some RV sites actually have a water view; else `none`. Research the RV-site-to-water relationship (campground site maps, per-site listings, photos, satellite) rather than trusting "on the river" marketing.
...
**Private (commercial) campgrounds:** add only if RV Life `price_level` ≤ 2 ($ or $$) AND `star_rating` ≥ 4 AND not a membership park; then vet each per the inclusion criteria (real transient RV campground, not seasonal/residential/workforce/club; dead-website strike) and set waterfront from the RV sites.
...
- **`waterfront` vocabulary:** on-water `lake`/`river`/`creek`/`pond`/`bay` (+ `coastal woods`/`coastal dunes`); view-only `lakeview`/`riverview`/`bayview`; else `none` (RV-sites-only rule, above).
