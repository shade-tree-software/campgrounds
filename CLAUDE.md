...
- **Waterfront reflects RV-accessible sites only, and an on-water value must be *earned* with site-level proof.** Riverfront/lakefront cabins or tent-only/shelter sites do NOT qualify. The standard is a burden of proof, not a vibe — follow it exactly:
  ...
  - **The satellite look is MANDATORY and decisive for every on-water value.** Pull the campground coordinate and actually look: do RV pads touch the water? This objective check overrides all prose and brochure maps and collapses the ambiguous cases. No satellite (or official site-map) confirmation → not an on-water value.
