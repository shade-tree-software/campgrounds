...
Admin + desktop-only feature on the trip detail page: the **Detect Stops** button in `.trip-header` scans the trip's GPS track for clusters of consecutive pings within `STOP_CLUSTER_RADIUS_M` (100 m) of a running centroid whose time span is at least `STOP_MIN_MINUTES` (5 min). The detector only considers pings whose local-date (per the ping's tz field) falls within the trip's `[start, end]` range. Clusters within `STOP_NEAR_ANCHOR_M` (300 m) of any existing stay, event (incl. waypoints), family location, *or HOME* are dropped — HOME is in the anchor set so pre-departure / post-arrival dwell at the house (which falls inside the date range but isn't a real trip stop) doesn't get suggested. Survivors are reverse-geocoded one-per-second via Nominatim (so the call can take tens of seconds; the modal shows a spinner) and classified: `duration_minutes <= STOP_WAYPOINT_MAX_MINUTES` (30 min) → waypoint, longer → event. Endpoint: `POST /api/trips/<id>/detect-stops` (advisory only; persists nothing). The modal lists each suggestion with a checkbox; the admin's selection is posted to `POST /api/trips/<id>/accept-stops`, which creates each via `add_event` with `needs_vetting: true` forced on (server-side) so it can't be smuggled off.
