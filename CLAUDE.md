...
**Waterfront audit is a built-in sweep stage, not a separate pass.** The original pre-gate re-audit (re-verifying old `waterfront` values state by state) is **COMPLETE**. For every new state from here on, fold the audit's full strength into the sweep so entries land correct the first time (chosen approach, KS-style 2026-06-11). Per-state pipeline:
  1. detect (RV Life) → vet inclusion criteria → pin `location` → fetch elevation;
  2. write the candidate entries with `waterfront: "not waterfront"` as a placeholder (don't guess from name/marketing);
  3. fan out the **same** `audit/waterfront_audit_instructions.md` subagents over the just-added ids (batches of ~8, waves of ~5) — the satellite look + per-site-map/rec.gov check + coord verification happen here at full strength;
  4. apply with `python3 audit/apply_waterfront_audit.py <results.json>` (sets `waterfront`, fixes mis-pinned `location`/`elevation_meters`);
  5. record per-entry `waterfront_evidence` in the commit message ("Audit all NN <ST> waterfront designations") so a later check is a `git log` grep — evidence lives in the commit, not a JSON field.
