...
- **Waterfront reflects RV-accessible sites only, and an on-water value must be *earned* with site-level proof.** Riverfront/lakefront cabins or tent-only/shelter sites do NOT qualify. The standard is a burden of proof, not a vibe — follow it exactly:
  ...
  - **Sandy beaches & dunes count as on-water — beach width and drawdown do NOT downgrade.** A pad sitting on an open sandy beach, or on the beach's dune/apron, is `lakefront` (or `bayfront`/`coastal dunes` as fitting) EVEN IF a wide band of sand lies between the pad and the current waterline, and even when that width is exposed lakebed from reservoir drawdown. The beach is itself the waterfront, so the "~1 pad-depth" test is measured to the **beach/high-water edge, not the drawn-down waterline**. Scattered trees that don't meaningfully block the view or beach access do not disqualify. What DOES disqualify is a genuine separating **buffer**: a continuous treeline/vegetated screen, a developed strip (road / parking / cabins) between sites and beach, or an elevated **bluff/bench** the site sits atop with a graded slope down to the shore (those resolve to `lakeview` if the open view holds, else `not waterfront`). So at a drawn-down reservoir, judge by whether the site sits on the open beach/dune apron — not by metres-to-waterline. (This corrected a CO sweep that read low-pool tiles as a wide beach "buffer" and wrongly defaulted reservoir beach loops down — e.g. Jackson Lake.)
