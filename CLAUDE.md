...
- **Manual ping overrides (admin):** trips carry two optional `tst`-keyed override lists, both applied server-side in `_apply_overrides` inside `api_trip_track`:
  ...
  - `relocated_pings: [{tst, lat, lon}, ...]` — the endpoint rewrites each matching ping's lat/lon to the override target before returning, so the polyline flows through the new positions. The "Relocate pings" mode (mutually exclusive with Select pings) reinterprets the lasso: on mouseup, every ping inside is POSTed with the circle's center as its new lat/lon. Re-relocating an existing entry replaces its target (so a second lasso can pick up a mix of fresh and previously-moved pings, all snapping to the new center). Render layer: amber `circleMarker`s with dashed lines back to the original coords.
